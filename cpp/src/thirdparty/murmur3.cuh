...
__device__ static inline FORCE_INLINE uint64_t getblock(const uint8_t* p, int o)
{
  uint64_t res;
  memcpy(&res, p + o, sizeof(uint64_t));
  return res;
}
...
__device__ void MurmurHash3_x64_128(const void* key, const int len, const uint32_t seed, void* out)
{
  ...
  for (i = 0; i < nblocks; i++) {
    uint64_t k1 = getblock(data, i * 16);
    uint64_t k2 = getblock(data, i * 16 + 8);
