                        ...
                        rmm::device_uvector<char>& workspace)
{
  ...
  raft::cluster::kmeans::cluster_cost(
    ...
    workspace,
    clusterCost.view(),
    [] __host__ __device__(const DataT& a, const DataT& b) { return a + b; });
  ...
  for (int iter = 0; iter < niter; ++iter) {
    ...
    rmm::device_uvector<size_t> nPtsSampledByRankVec(n_rank, stream);
    ...
    RAFT_CUDA_TRY(cudaMemsetAsync(nPtsSampledByRank, 0, n_rank * sizeof(size_t), stream));

    size_t nPts = inRankCp.size() / n_features;
    raft::copy(nPtsSampledByRank + my_rank, &nPts, 1, stream);
    ...
    thrust::transform(handle.get_thrust_policy(),
                      ...
                      nPtsSampledByRank,
                      [=] __host__ __device__(size_t val) { return val * n_features; });
    raft::update_host(rank_sizes.data(), nPtsSampledByRank, (size_t)n_rank, stream);
    handle.sync_stream();
                  ...
                  raft::device_vector_view<DataT, IndexT> weight)
{
  ...
  if (wt_sum != n_samples) {
    ...
    raft::linalg::unaryOp(
      ...
      weight.data_handle(),
      weight.size(),
      [=] __host__ __device__(const DataT& wt) { return wt * scale; },
      stream);
  }
}
         ...
         rmm::device_uvector<char>& workspace)
{
  ...
  for (n_iter[0] = 1; n_iter[0] <= params.max_iter; ++n_iter[0]) {
    ...
    raft::linalg::matrixVectorOp(
      ...
      true,
      false,
      [=] __host__ __device__(DataT mat, DataT vec) {
        if (vec == 0)
          return DataT(0);
        else
          return mat / vec;
    ...
    raft::matrix::gather_if(
      ...
      newCentroids.data_handle(),
      [=] __host__ __device__(raft::KeyValuePair<ptrdiff_t, DataT> map) {  // predicate
      ...
      [=] __host__ __device__(raft::KeyValuePair<ptrdiff_t, DataT> map) {  // map
        return map.key;
      },
      stream);
    ...
    raft::linalg::mapThenSumReduce(
      ...
      newCentroids.size(),
      [=] __host__ __device__(const DataT a, const DataT b) {
        DataT diff = a - b;
        return diff * diff;
      },
      stream,
    ...
    if (params.inertia_check) {
      ...
      raft::cluster::kmeans::cluster_cost(
        ...
        [] __host__ __device__(const raft::KeyValuePair<IndexT, DataT>& a,
         ...
         raft::host_scalar_view<IndexT> n_iter)
{
  ...
  fit<DataT, IndexT>(
    handle, params, data, weight.view(), centroidsRawData.view(), inertia, n_iter, workspace);
