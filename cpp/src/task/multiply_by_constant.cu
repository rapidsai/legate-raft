...
struct multiply_by_constant_fn_cpu {
...
__global__ void multiply_by_constant_kernel(value_t* out,
                                            const value_t* in,
                                            value_t value,
                                            size_t volume)
{
...
template __global__ void multiply_by_constant_kernel(float*, const float*, float, size_t);
template __global__ void multiply_by_constant_kernel(double*, const double*, double, size_t);

struct multiply_by_constant_fn_gpu {
  template <legate::Type::Code CODE,
            int32_t DIM,
                  ...
                  legate::Scalar value)
  {
    ...
    multiply_by_constant_kernel<<<num_blocks, block_size, 0, stream>>>(
 ...
 public:
  static void cpu_variant(legate::TaskContext context)
  {
    ...
    legate::double_dispatch(
      input.data().dim(), input.data().code(), multiply_by_constant_fn_cpu{}, output, input, value);
  }
