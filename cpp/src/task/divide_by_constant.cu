...
struct divide_by_constant_fn_cpu {
...
__global__ void divide_by_constant_kernel(value_t* out,
                                          const value_t* in,
                                          value_t value,
                                          size_t volume)
{
...
template __global__ void divide_by_constant_kernel(float*, const float*, float, size_t);
template __global__ void divide_by_constant_kernel(double*, const double*, double, size_t);

struct divide_by_constant_fn_gpu {
  template <legate::Type::Code CODE,
            int32_t DIM,
                  ...
                  legate::Scalar value)
  {
    ...
    divide_by_constant_kernel<<<num_blocks, block_size, 0, stream>>>(
...
class DivideByConstantTask : public Task<DivideByConstantTask, DIVIDE_BY_CONSTANT> {
 public:
  static void cpu_variant(legate::TaskContext context)
  {
    ...
    legate::double_dispatch(
      input.data().dim(), input.data().code(), divide_by_constant_fn_cpu{}, output, input, value);
  }
...
{
  legate_raft::DivideByConstantTask::register_variants();
}
